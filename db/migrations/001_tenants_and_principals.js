// db/migrations/001_tenants_and_principals.js
//
...
//
//     key_version    — incremented on key rotation. Scopes the Redis key cache.
//                      Cache key: `tenant:{id}:key:{key_version}`
//
//     policy_version — incremented on any role/permission/policy change.
//                      Scopes the Redis decision cache.
//                      Cache key: `tenant:{id}:policy:{policy_version}:principal:{principal_id}:v:{principal_version}`
//
//   They are independent by design. A key rotation MUST NOT flush the decision
//   cache. A policy change MUST NOT flush the key cache. (AD-P-07)
//
// PRINCIPALS
//   A user or service account within a tenant. external_id is the tenant's own
...
export async function up(pgm) {
  ...
  pgm.createTable("tenants", {
    ...
    // Validation happens in the adapter before write, in IdentityService at read.
    public_key: {
      type: "text",
      notNull: true,
    },
    ...
    // Scopes the Redis key cache entry so in-flight requests complete against
    // the key they resolved at the start of validation (AD-P-07a).
    // Starts at 1 — 0 is never a valid live value.
    key_version: {
      type: "integer",
      notNull: true,
      default: 1,
    ...
    // PolicyRegistry on any policy change. Scopes the Redis decision cache.
    // Independent from key_version — rotating a key does not invalidate decisions. (AD-P-07)
    policy_version: {
      type: "integer",
      notNull: true,
      default: 1,
